...
model_path = os.getenv("MODEL_PATH", "/runpod-volume/models/deepsex-34b.Q4_K_M.gguf")
book_dir = os.getenv("BOOK_DIR", "/runpod-volume/book")
...
def handler(event, context):
    """
    Обработчик запросов RunPod Serverless.
    Ожидаемый формат event['body']:
    {
        "plot": "Текст сюжета",
        "chapter_prompt": "Промпт для главы",
        "chapter_number": 1,
        "custom_vocab": ["страсть", "ETH/HYN", "TradingView"],
        "max_length": 1000,
        "temperature": 0.7
    }
    """
